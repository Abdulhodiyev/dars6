...
select full_name, salary (select avg(salary) from staff) as avg_sum staff;

--Vazifa 4: Har bir bo'lim bo'yicha xodimlar sonini va ularning umumiy maoshlarini aniqlash so'rovini yozing.
select count(*) as xodimlar_soni, (select sum(salary) from staff) as total_salary from staff;
...
select position, avg(experience_years) as avg_experience from staff group by position;
...
select * from staff where experience_years = (select min(experience_years) from staff);

--Vazifa 9: Har bir yilda va har bir bo'lim bo'yicha ishga qabul qilingan xodimlar sonini ko'rsatish so'rovini yozing.
select count(*) as xodimlar_soni, hire_year, department
from staff group by hire_year, department order by hire_year;
...
select * from staff where salary > ( select avg(salary) from staff) and experience_years < 5;

--Vazifa 11: Har bir lavozim bo'yicha eng yuqori va eng past maoshga ega bo'lgan xodimlarni aniqlash so'rovini yozing.
select position, max(salary) as max_salay, min(salary) as min_salary
from staff group by position order by position;

--Vazifa 12: Har bir bo'limda eng ko'p xodim ishlaydigan lavozimni aniqlash so'rovini yozing.
select department, count(*) as xodimlar_soni from staff group by department;
